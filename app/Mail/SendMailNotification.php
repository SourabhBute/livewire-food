...
class SendMailNotification extends Mailable
{
  ...
  /**
    ...
    public function Convert($mail_types)
    {
        ...
        $this->variableValues["header"] = File::get(app()->langPath().'/'.app()->getLocale().'/mailer/header.tpl');
        $this->variableValues["footer"] = File::get(app()->langPath().'/'.app()->getLocale().'/mailer/footer.tpl');
        ...
        $content = File::get(app()->langPath().'/'.app()->getLocale().'/mailer/'.$mail_types['code'].'.tpl');
