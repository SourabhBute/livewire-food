...
class Edit extends Component
{
    ...
    public $cuisines;
    public $cuisine_values;
    public $enable_scheduler;
    ...
    public function mount($id) {
        ...
        $this->cuisine_values  = explode(', ', $this->store->cuisine); 
        ...
        $this->enable_scheduler = $this->store->enable_scheduler;
        
    }

    public function storeOpenStatus() {
        
        if($this->bussinessHours){
            $openStatus = Arr::first($this->bussinessHours, function ($value, $key) {
                    return $value["status"] == 1;
            });

            if($openStatus) {
                Store::where('id', '=' , $this->store->id)->update(['is_open' => 1]);
                $this->store->is_open = 1;
            }else {
                
                Store::where('id', '=' , $this->store->id)->update(['is_open' => 0]);
                $this->store->is_open = 0;
            }
        }
    }
    ...
    public function updatedEnableScheduler(){
        $this->enable_scheduler = $this->enable_scheduler ? 1 : 0;   
        
        $this->store->update(['enable_scheduler' => $this->enable_scheduler]);
        
        $this->dispatchBrowserEvent('alert', 
        ['type' => 'success',  'message' => __('store.Enable scheduler successfully updated.')]);
    }
    ...
    public function editBussinessHour($index) {
        ...
        if(!is_null($bussinessHour)) {
            ...
            if($editedBussinessHour) {
                ...
                $this->emit('storeOpenStatus');
                $this->dispatchBrowserEvent('alert', 
                ['type' => 'success',  'message' => 'Business Hours changed Successfully!']);
            }
        }
    ...
    public function update() {
        ...
        $this->store->cuisine = implode(', ', $this->cuisine_values); 
   ...
   /**
     ...
     * @return response()
     */
    public function updatedBackgroundImagePath()
    {   
        $validator = Validator::make(
            ['background_image_path' => $this->background_image_path],
            ['background_image_path' => 'mimes:jpg,jpeg,png|required|max:1024'],
        );

        if ($validator->fails()) {
            $this->reset('background_image_path');
            $this->setErrorBag($validator->getMessageBag());
            return redirect()->back();
        }

            //Upload original
            $this->background_image_path->store('/stores/original', config('app_settings.filesystem_disk.value'));
        
            //Upload thumbnails
            $filePath = '/stores/thumbnails/' . $this->background_image_path->hashName();  
            $image = Image::make($this->background_image_path)->resize(728, null, function ($constraint) {
                $constraint->aspectRatio();  
                $constraint->upsize();               
            })->stream();
            ...
            Store::where('id', '=' , $this->store->id )->update(['background_image_path' =>  $filePath]);
        
         $this->dispatchBrowserEvent('alert', 
        ['type' => 'success',  'message' => __('store.Banner image changed successfully!')]); 
   }
    ...
    {   
        ...
        $status = ( $store['application_status'] == 'suspended'  ) ? 1: 0;
        ...
        if($storeDatas) {
            foreach($storeDatas as $storeData ) {
                if($application_status == "suspended") {
                    event(new InstantMailNotification($storeData["user_id"], [
        ...
        if($application_status == 'suspended') {
            $this->dispatchBrowserEvent('alert-bottom', 
            ['type' => 'success',  'message' => __('store.Account is suspended successfully!')]);
        }
   }
   ...
   public function editTranslate()
    ...
    public function getCuisineForInput($value){ 
        $this->cuisine_values = $value;
    }
