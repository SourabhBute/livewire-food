...
class Create extends Component
{
    ...
    protected $rules = [
            ...
            'price_sale' => 'nullable|numeric|price:price_sale',           
            'sale_start_date' => 'nullable|required_with:price_sale|date',
            'sale_end_date' =>'nullable|required_with:price_sale|date|after_or_equal:sale_start_date', 
    ...
    public function messages() {
        return [
            ...
            "price.required" => __('product.The actual price field is required.'),
            "price_sale.numeric" => __('product.The sale price must be a number.'),
            "price.regex" => __('product.The actual price format is invalid.'), 
            'categories_ids.required' => __("product.The category field is required."),
            'store_id.required' => __("product.The store field is required."),
    ...
    public function store() {
        if(auth()->user()->hasRole('Admin')) {
            $validateStore = ["store_id" => "required"];
            $adminValidate = array_merge($this->rules, $validateStore);
            $this->validate($adminValidate);
