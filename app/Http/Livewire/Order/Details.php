...
class Details extends Component
{
    ...
    {    
        try {
            $mid = $this->order->order_number;
            $store_name =  $this->order->store->name;
            $store_address = $this->order->store->storeAddress->address_line_1;
            $store_phone = $this->order->store->phone;
            $store_email = $this->order->store->email;
            $store_website = " ";

            $currency = config('app_settings.currency_symbol.value');
            $tax = $this->order->tax_amount;
            $itemAmounts = $this->order->sub_total_amount;
            ...
            $printer->setTax($tax);
            // Set request amount
            $printer->setRequestAmount($itemAmounts);
            ...
            return "Printer has not been initialized". $e->getMessage();  
        }
    }
