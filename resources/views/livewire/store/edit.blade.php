        ...
        <div class="col-lg-3">
            <div class="card position-sticky top-1">
                <ul class="nav flex-column bg-white border-radius-lg p-3">
                    ...
                    <li class="nav-item pt-2">
                        <a class="nav-link text-dark d-flex" data-scroll="" href="#cuisines">
                            <i class="material-icons text-lg me-2">cookie</i>
                            <span class="text-sm">Cuisines</span>
                        </a>
                    </li>
                    ...
                    <li class="nav-item pt-2">
                        <a class="nav-link text-dark d-flex" data-scroll="" href="#storeSettings">
            ...
            <div class="card card-body" id="profile">
                <div class="row justify-content-center align-items-center">
                    ...
                    <div class="col-sm-auto col-4">
                            ...
                            <div class="avatar avatar-xl position-relative preview"  wire:key="foo">
                                @if($logo_path)
...
<!--                         
          ...
          <!-- Cuisines info -->
          <div class="card mt-4" id="cuisines">
            <div class="card-header">
                <h5>Cuisines</h5>
            </div>
            <div class="card-body pt-0">                
                <div class="row">
                   <div class="col-12 mb-4">
                        <div class="input-group input-group-static">
                            ...
                            <select multiple="multiple"  class="form-control" wire:model="cuisine_values"
                                    data-style="select-with-transition" title="Cuisines" data-size="100"  id="cuisine_values">
       ...
       <div class="card mt-4" id="preparing-time">
                ...
                <x-table>
                    ...
                    <x-slot name="body">
                        ...
                        <x-table.row wire:key="row-{{ $account->id }}">
                            ...
                            <x-table.cell><a href="{{ route('view-user',  $account->user) }}">+{{$account->user->country_code}} {{ substr($account->user->phone , +(strlen($account->user->country_code)))  }}</a></x-table.cell>
                            ...
                            <x-table.cell>{{ $account->created_at->format(config('app_settings.date_format.value')) }}</x-table.cell>
                            <x-table.cell>
        ...
        <div class="card mt-4" id="bussinessHours">
                                ...
                                <div class="input-group input-group-static mb-1 text-center"> 
                                    <select wire:change="changeOpeningTime({{$mainKey}},$event.target.value)" wire:model.lazy="bussinessHours.{{$mainKey}}.opening_time"  class="form-control" id="OpeningTime">
                                      @foreach ($timeOptionsList as $openingKey => $openingTime)
                                        <option  value="{{$openingKey}}" class="text-center" {{ ($bussiness["status"]==0)? 'disabled' : '' }}>{{$openingTime}}</option>
                                      @endforeach
                                    </select>
                                ...
                                <div class="input-group input-group-static mb-1 text-center"> 
                                    <select wire:change="changeClosingTime({{$mainKey}},$event.target.value)" wire:model.lazy="bussinessHours.{{$mainKey}}.closing_time"   class="form-control" id="OpeningTime">
                                        @foreach ($timeOptionsList as $closingKey => $closingTime)
                                            <option value="{{$closingKey}}" class="text-center" {{ ($bussiness["status"]==0)? 'disabled' : '' }} >{{$closingTime}}</option>
                                        @endforeach
                                </select>
          ...
          <!-- commission info Time -->
          <div class="card mt-4" id="storeSettings">
            <div class="card-header">
                <h5>Settings</h5>
            </div>
            <div class="card-body pt-0">                
                <div class="row">
                    ...
                    <div class="col-12  mb-4">                    
                        <div class="form-check">
                            <input  wire:loading.attr="disabled"  wire:model.lazy="enable_scheduler" class="form-check-input" type="checkbox"  id="enable_scheduler">
                    ...
                    <div class="col-12 mb-4">
                        <div class="input-group input-group-static">
                            <div class="avatar avatar-xxl m-2 position-relative ">
                                <div class="position-relative preview">
                                    ...
                                    <label for="file-input"
                                        ...
                                        <i wire:loading.remove class="fa fa-pen top-0" data-bs-toggle="tooltip" data-bs-placement="top"
                                            title="" aria-hidden="true" data-bs-original-title="Change Banner Image"
                                            aria-label="Change Banner Image"></i><span class="sr-only">Change Banner Image</span>
                                            <div wire:loading wire:target="background_image_path">
                                                <x-spinner></x-spinner>
                                            </div>
                                    </label>
                                    <span class="h-12 w-12 rounded-full overflow-hidden bg-gray-100 ">
                                        @if ($background_image_path)
                                            <img src="{{ $background_image_path->temporaryUrl() }}" alt="Profile Photo">
                                        @elseif ($store->background_image_path)                                        
                                            <img src="{{ Storage::disk(config('app_settings.filesystem_disk.value'))->url($store->background_image_path)}}" alt="Photo">
                                        @else
                                            <img src="{{ asset('assets') }}/img/default-food-avatar.jpg" alt="avatar">
                                        @endif
                                    </span>
                                    <input wire:loading.attr="disabled" wire:model="background_image_path" type="file" id="file-input">
                                </div>
                            </div>
                        </div>
...
<script>   
    $(document).ready(function() { 
        ...
        window.initSelectCuisine=()=>{  
            $('#cuisine_values').select2({
        ...
        $('#cuisine_values').on('change', function (e) {  
            var selected_element = $(e.currentTarget);
            var select_val = selected_element.val();   console.log('select_val', select_val);
            
            window.livewire.emit('getCuisineForInput', select_val);
        });
